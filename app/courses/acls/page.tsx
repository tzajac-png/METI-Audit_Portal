...
export default async function AclsCoursesPage() {
  try {
    const { courses, fetchedAt } = await fetchProgramCourseSummaries("ACLS");
    ...
    return (
      <div className="rounded-xl border border-red-900/40 bg-red-950/20 p-8">
        <h1 className="text-xl font-semibold text-red-400">
          Could not load ACLS courses
        </h1>
        <p className="mt-2 text-zinc-300">{message}</p>
        <p className="mt-4 text-sm text-zinc-500">
          Confirm the ACLS Instructor tab is shared for link viewing. Optional
          env:{" "}
          <code className="rounded bg-zinc-900 px-1 text-red-300">
            GOOGLE_SHEET_GID_ACLS_INSTRUCTOR
          </code>
        </p>
      </div>
    );
